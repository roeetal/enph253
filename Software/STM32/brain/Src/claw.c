...
void actuatengo(TIM_HandleTypeDef *htim, uint16_t channel_claw, uint16_t channel_arm)
{
    ...
    open_claw(htim);
    // Lower arm gently
    slow_actuate(htim, channel_arm, 0, 180);
    HAL_Delay(1000);
    // Open claw again
    open_claw(htim);
}
...
{
    actuate(htim, TIM_CHANNEL_1, 140);
    HAL_Delay(1000);
}
...
void open_claw(TIM_HandleTypeDef *htim)
{
    ...
    HAL_Delay(200);
    actuate(htim, TIM_CHANNEL_1, 60);
    HAL_Delay(200);
}

void arm_down(TIM_HandleTypeDef *htim)
{
    actuate(htim, TIM_CHANNEL_2, 130);
    HAL_Delay(1000);
}
...
{
    actuate(htim, TIM_CHANNEL_2, deg);
    HAL_Delay(1500);
}
